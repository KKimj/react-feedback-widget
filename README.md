...
> - 🖼 **스크린샷 2종** — 클릭한 요소(셀렉터) + 전체화면을 각각 캡처·업로드, 시트에 `=IMAGE` 2컬럼 + URL 2컬럼
> - 🇰🇷 **한글 UI** — 모달·카테고리·우선순위
...
> - 📄 **시트 컬럼** — `ID·우선순위·원문·제보자·상태·담당자·작업내용·요소·셀렉터·링크·스크린샷(셀렉터)·스크린샷(전체화면)·셀렉터 URL·전체화면 URL` (제보자 = 위젯 `userName`/`userEmail` prop)
...
> node init-sheet.mjs           # (SA 있을 때) 시트 헤더·서식·드롭다운 초기화 — 최초 1회
